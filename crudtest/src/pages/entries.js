...
export default function Entries() {
  ...
  const numberVisited = page * entriesPerPage;
  ...
  useEffect(() => {
    ...
    axios
      .get('/getentries')
      .then((result) => {
        console.log('entry fetch', result);
      ...
      {loading ? (
        ...
        <>
          <GridWrapper container justifyContent={'center'} spacing={3}>
            ...
            {entries
              ? entries
                  .slice(numberVisited, numberVisited + entriesPerPage)
                  .map((entry) => {
                    return (
                      <Grid item xs={12} md={3} key={entry._id}>
                        <Card>
                          <List>
